...
import { paginOfMogalIndeFilm } from './library-pagination';

import * as basicLightbox from 'basiclightbox';
import 'basiclightbox/dist/basicLightbox.min.css';

const addToFirebases = new AddToFirebase();

const refs = {
  backdrop: document.querySelector('.backdrop'),
  modalMovies: document.querySelector('[data-modal-lib]'),
  openModalCard: document.querySelector('[data-modal-open-lib]'),
  closeModalBtn: document.querySelector('[data-modal-close-lib]'),
  trailerBtn: document.querySelector('.trailer-btn'),
  watchedLibraryBtn: document.querySelector('.js-btn-library-watched'),
  body: document.querySelector('.bodygart'),
};
...
export function openModal(evt) {
  ...
  function handleWathedBtnClick() {
    ...
    if (!arrWatched.some(film => film.id === activeFilm.id)) {
      ...
      fetchModal(currentId)
        .then(data => {
          addToFirebases.addMovieToFireBase(data, 'Watched');
        })
        .catch(error => console.log(error));
    }
  ...
  function handleQueueBtnClick() {
    ...
    if (!arrQueue.some(film => film.id === activeFilm.id)) {
      ...
      fetchModal(currentId)
        .then(data => {
          addToFirebases.addMovieToFireBase(data, 'Queue');
        })
        .catch(error => console.log(error));
    }
  }
  function handleRemoveQueue() {
    ...
    if (arrQueue.some(film => film.id === activeFilm.id)) {
      ...
      fetchModal(currentId)
        .then(data => {
          addToFirebases.deleteMovieFromFireBase(data, 'Queue');
        })
        .catch(error => console.log(error));
    }
  }
  function handleRemoveWatched() {
    ...
    if (arrWatched.some(film => film.id === activeFilm.id)) {
      ...
      fetchModal(currentId)
        .then(data => {
          addToFirebases.deleteMovieFromFireBase(data, 'Watched');
        })
        .catch(error => console.log(error));
    }
  }
